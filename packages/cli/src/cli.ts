...
// Plugin commands
const pluginCmd = program.command("plugin").alias("p").description("Manage Yukemuri plugins")
...
// Create commands
const createCmd = program.command("create").alias("c").description("Create new Yukemuri components")
...
// Dev commands
const devCmd = program.command("dev").alias("d").description("Development tools")
...
program
  ...
  .action(async (name: string | undefined, options: Record<string, unknown>) => {
    ...
    await createProject.parseAsync([
      "node",
      "yukemuri",

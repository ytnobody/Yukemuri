...
export class CreateCommands {
  ...
  static project(): Command {
    return new Command('project')
      ...
      .action(async (name: string | undefined, options: ProjectOptions) => {
        ...
        try {
          await this.generateProject(answers, !options.install);
  ...
  static api(): Command {
    return new Command('api')
      ...
      .action(async (apiPath: string, options: ApiOptions) => {
        ...
        try {
          await this.generateApiRoute(apiPath, options);
  ...
  // Helper methods
  private static async generateProject(options: ProjectAnswers, skipInstall: boolean): Promise<void> {
    const projectDir = path.join(process.cwd(), options.name);
    
    // Clone template from create-yukemuri package
    const templatePath = path.join(__dirname, '..', '..', '..', 'create-yukemuri', 'templates', options.template);
    
    if (await fs.pathExists(templatePath)) {
      await fs.copy(templatePath, projectDir);
    } else {
      // Fallback to base template
      const baseTemplatePath = path.join(__dirname, '..', '..', '..', 'create-yukemuri', 'templates', 'base');
    ...
    await this.updateYukemuriConfig(projectDir, options.features);
...
`;
    ...
    await this.addRoute(routePath, fileName);
  }
...
`;
  ...
  private static async generateApiRoute(apiPath: string, options: ApiOptions): Promise<void> {
    ...
    const methods = (options.methods || 'GET,POST').split(',').map((m: string) => m.trim().toUpperCase());

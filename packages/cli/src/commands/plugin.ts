...
export class PluginCommands {
    ...
    return new Command('add')
      ...
      .action(async (plugin: string, options: AddOptions) => {
        ...
        try {
          ...
          if (!options.noConfig) {
            await this.setupPluginConfig(plugin);
          }
    ...
    return new Command('info')
      ...
      .action(async (plugin: string) => {
        try {
          ...
          const packageInfo = await this.getPluginPackageInfo(plugin);
          ...
          const configSchema = await this.getPluginConfigSchema(plugin);
          if (configSchema) {
            console.log(chalk.green('⚙️ Configuration Schema:'));
            this.printConfigSchema(configSchema);
          }
    ...
    return new Command('search')
      ...
      .action(async (query: string, options: { limit?: string }) => {
        ...
        try {
          ...
          const results = await this.searchPlugins(query, limit);
  ...
  static create(): Command {
    return new Command('create')
      ...
      .action(async (name: string | undefined, options: { template?: string }) => {
        ...
        try {
          await this.generatePlugin(answers);
  ...
  private static async generatePlugin(options: InquirerAnswers): Promise<void> {
    ...
    const pluginSource = this.generatePluginSource(options);
    await fs.writeFile(path.join(pluginDir, 'src', 'index.ts'), pluginSource);
    
    // Generate README
    const readme = this.generatePluginReadme(options);

    ...
    return new Command("server")
      ...
      .action(async (options: ServerOptions) => {
        ...
        try {
          ...
          console.log(
            chalk.gray(
              `📡 Server will be available at http${options.https ? "s" : ""}://${options.host}:${options.port}`
            )
          )
      ...
      .action(async (options: DeployOptions) => {
        ...
        try {
          switch (options.platform) {
            case "cloudflare":
              await DevCommands.deployCloudflare(options)
              break
            case "vercel":
              await DevCommands.deployVercel(options)
              break
            case "netlify":
              await DevCommands.deployNetlify(options)
              break
            default:
  ...
  static generateTypes(): Command {
    return new Command("generate-types")
      ...
      .action(async (options: GenerateTypesOptions) => {
        ...
        try {
          if (options.api) {
            await DevCommands.generateApiTypes()
            spinner.text = "Generating API types..."
          }

          if (options.db) {
            await DevCommands.generateDatabaseTypes()

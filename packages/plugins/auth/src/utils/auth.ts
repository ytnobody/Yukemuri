...
export class JWTUtils {
  ...
  static sign(payload: Record<string, any>, secret: string, expiryTime: string = '7d'): string {
    ...
    const expirySeconds = this.parseExpiryTime(expiryTime);
    const tokenPayload = {
      ...payload,
      iat: now,
...
export class OAuthMockProvider {
   ...
   */
  static verifyToken(provider: string, token: string, _clientSecret?: string): Record<string, any> | null {
   ...
   */
  static getAuthUrl(provider: string, clientId: string, redirectUri: string, scope: string[]): string {

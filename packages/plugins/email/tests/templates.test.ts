...
test('TemplateEngine - Render template with missing variables should fail', async (t) => {
  ...
  try {
    ...
    assert.match((error as Error).message, /Missing template variables/, 'Should mention missing variables');
  }
});
...
test('TemplateEngine - Render nonexistent template should fail', async (t) => {
  ...
  try {
    ...
    assert.ok(error instanceof Error, 'Should throw Error');
    assert.match((error as Error).message, /Template not found/, 'Should mention template not found');
  }
});
...
test('TemplateEngine - Welcome template verification', async (t) => {
  ...
  const template = engine.get('welcome');
  assert.ok(template, 'Welcome template should exist');
  assert.deepEqual(template?.variables, ['firstName', 'appName', 'dashboardUrl'], 'Should have correct variables');
